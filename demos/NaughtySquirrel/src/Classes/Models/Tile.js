...
var Tile = cc.Layer.extend({
  ...
  ctor: function(type, index, offset, track) {
    ...
    if (index == 1) {
      ...
      this.setPosition(cc.p(WIN_SIZE.width / 2, WIN_SIZE.height));
      this.distance = TILE.SIZE / 2 + WIN_SIZE.height;

    } else {
      this.setPosition(cc.p(WIN_SIZE.width / 2, WIN_SIZE.height + TILE.SIZE / 2 + offset));
    ...
    this.tileBackground.setAnchorPoint(cc.p(0.5, 0.5));
    this.tileBackground.setPosition(cc.p(0, 0));
  ...
  goStraight: function() {
    ...
    this.goStraightAction = cc.MoveTo.create(
      ...
      cc.p(WIN_SIZE.width / 2, -TILE.SIZE / 2)
    );
