...
var Track = cc.Layer.extend({
  ...
  loadCharacter: function() {
    ...
    this.character.setPosition(cc.p(WIN_SIZE.width / 2, WIN_SIZE.height / 4));
  ...
  loadScoreLayer: function() {
    ...
    this.addChild(this.scoreLayer, 10);

    this.scoreLayer.generatePanel([{
      type: 'trophies',
      position: cc.p(LOCATION.GAME_LAYER.TROPHIES.X, LOCATION.GAME_LAYER.TROPHIES.Y),
    }, {
      type: 'distance',
      position: cc.p(LOCATION.GAME_LAYER.DISTANCE.X, LOCATION.GAME_LAYER.DISTANCE.Y),
    }]);
  ...
  onTouchesEnded: function(touches, event) {
    ...
    if(touches[0]._point.x < WIN_SIZE.width / 2 &&
       Math.abs(touches[0]._point.x - TOUCHES.x) < 100 &&
       Math.abs(touches[0]._point.y - TOUCHES.y) < 100 && 
       ((actionPermissions.ACTION_MOVE_LEFT && this.character.isRunning()) || 
       (actionPermissions.ACTION_JUMP_LEFT && this.character.isJumping()))) {
        this.character.moveLeft();
    } else if (touches[0]._point.x >= WIN_SIZE.width / 2 &&
               Math.abs(touches[0]._point.x - TOUCHES.x) < 100 &&
               Math.abs(touches[0]._point.y - TOUCHES.y) < 100 && 
               ((actionPermissions.ACTION_MOVE_RIGHT && this.character.isRunning()) || 
               (actionPermissions.ACTION_JUMP_RIGHT && this.character.isJumping()))) {
        this.character.moveRight();
    } else if (touches[0]._point.x - TOUCHES.x > 100) {
        this.keyRightHandler(distance, actionPermissions);
    } else if (touches[0]._point.x - TOUCHES.x < -100) {
        this.keyLeftHandler(distance, actionPermissions);
    } else if (touches[0]._point.y - TOUCHES.y > 100) {
        this.keyUpHandler();
    }
...
var TrackScene = cc.Scene.extend({
    onEnter:function () {
        this._super();
        var layer = new Track();
        this.addChild(layer);
